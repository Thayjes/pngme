...
impl Png {
    ...
    pub fn as_bytes(&self) -> Vec<u8> {
        ...
        println!("chunks length {}", self.chunks.len());
...
impl std::fmt::Display for Png {
        fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
            ...
            let mut chunk_type_strings : Vec<String> = Vec::new();
            for chunk in self.chunks.iter() {
                chunk_type_strings.push(chunk.chunk_type.to_string());
            }
            write!(f, "{}", chunk_type_strings.join(", "))?;
            Ok(())
        }
}
...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(bytes: &[u8]) -> Result<Self, Self::Error> {
        // How would you check the header first?
        if bytes.len() < 8{
            return Err("Not enough bytes for png header");
        }

        if &bytes[0..8] != &Png::STANDARD_HEADER{
            return Err("Header bytes mismatch");
        }

        let mut position = 8;
        let mut chunks = Vec::new();
        while position + 12 <= bytes.len(){
            let length_bytes = &bytes[position..position+4];
            let length_arr = [length_bytes[0], length_bytes[1], length_bytes[2], length_bytes[3]];
            let length_u32 = u32::from_be_bytes(length_arr);
            let length_usize = length_u32 as usize;

            let chunk_bytes = &bytes[position..position+12+length_usize];
            let chunk = Chunk::try_from(chunk_bytes)?; // Handle the Result
            chunks.push(chunk);

            position += 12 + length_usize; // Move to next chunk    }
        }
